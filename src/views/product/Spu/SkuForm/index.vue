  ...
  <div>
    <el-form ref="form" label-width="80px">
      ...
      <el-form-item label="价格(元)">
        <el-input-number v-model="skuInfo.price" placeholder="价格(元)"></el-input-number>
      </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      spu: {},
      // 收集图片的数据字段：但是需要注意的是，收集的数据目前缺少isDefault字段，将来提交给服务器数据的时候，需要整理参数（添加isDefault字段）
      imageList: []
    }
  },
  methods: {
    ...
    // 展示图片的table表格复选框按钮的事件
    handleSelectionChange(selection) {
      // 获取到用户选中图片的信息数据，但是需要注意的是，当前收集的数据中，缺少isDefalut字段
    ...
    async save() {
      ...
      // 整理平台属性
      const result = attrInfoList.reduce((prev, item) => {
      ...
      // 整理销售属性
      const result1 = spuSaleAttrList.reduce((prev, item) => {
        if (item.attrIdAndValueId) {
      ...
      // 发请求
      const result2 = await this.$API.spu.reqAddSku(skuInfo)
      if (result2.code === 200) {
